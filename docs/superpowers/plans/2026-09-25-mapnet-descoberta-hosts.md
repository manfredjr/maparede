...
Ramo `descoberta-hosts`. Desenho em `docs/superpowers/specs/2026-09-25-mapnet-design.md`.
...
|---|---|---|---|
| 1 | Sub-rede: máscara, prefixo, faixa de hosts, limite de /22 | `src/mapnet.nucleo/rede/sub-rede.cs` | `sub-rede-testes.cs` |
...
| 9 | Janela WinForms e modo linha de comando no mesmo `.exe` | `src/mapnet/*.cs` | Aberto no Wine; teste no Windows é do Manfred |
...
4. No Prompt de Comando: `mapnet --interfaces` e `start /wait mapnet --varrer --abrir`.
