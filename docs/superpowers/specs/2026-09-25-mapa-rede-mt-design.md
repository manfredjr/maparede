# MT Mapa de Rede: desenho da versão 1
...
- **Núcleo** (`src/mapa-rede-mt.nucleo`, `net8.0`): toda a lógica, sem janela. Testável no Windows e no Linux.
- **Aplicativo** (`src/mapa-rede-mt`, `net8.0-windows`): janela WinForms e ponto de entrada da linha de comando. Só chama o núcleo.
- **Testes** (`testes/mapa-rede-mt.testes`): xUnit sobre o núcleo. O ARP entra por interface (`ISondaArp`) para ser simulado.
...
- **Onde o dado fica:** só na máquina do técnico, em `Documentos\MT Mapa de Rede`. O programa não envia nada para fora da rede local e o repositório nunca recebe relatório.
- **Segredos:** o programa não usa senha, chave nem token. O certificado de assinatura de código, se vier, fica fora do repositório.

## 6. Ambiente

Windows 10 ou 11 de 64 bits, com o SDK do .NET 8 para compilar. O CI roda em Windows no GitHub Actions e guarda o `.exe` como artefato. Não há servidor.
