# MapaRede - MT: desenho da versão 1
...
| Nome | MapaRede - MT, executável `maparede.exe` | Decisão do Manfred em 25/09/2026 |
| Código | Aberto, repositório público `manfredjr/maparede`, licença GPL-3.0 | Decisão do Manfred em 25/09/2026, no mesmo modelo do CronoAula |
| Página | `maparede.manfred.com.br`, estática, na pasta `public/`, publicada pelo Git do cPanel | Decisão do Manfred em 25/09/2026. O `.exe` é baixado do GitHub Releases |

## 4. Arquitetura

- **Núcleo** (`src/maparede.nucleo`, `net8.0`): toda a lógica, sem janela. Testável no Windows e no Linux.
- **Aplicativo** (`src/maparede`, `net8.0-windows`): janela WinForms e ponto de entrada da linha de comando. Só chama o núcleo.
- **Testes** (`testes/maparede.testes`): xUnit sobre o núcleo. O ARP entra por interface (`ISondaArp`) para ser simulado.
...
- **Onde o dado fica:** só na máquina do técnico, em `Documentos\MapaRede - MT`. O programa não envia nada para fora da rede local e o repositório nunca recebe relatório.
