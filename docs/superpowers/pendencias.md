...
| Assinatura digital do `.exe` | Sem certificado de assinatura de código, o SmartScreen avisa em todo `.exe` baixado da internet ("Fornecedor desconhecido"), e o Smart App Control, onde estiver ativo, bloqueia sem oferecer saída. Algum antivírus também pode desconfiar de um programa que varre a rede. O ícone já está em DIB, que evita o bloqueio visto no CronoAula | Fechado em 26/09/2026: o Manfred decidiu não comprar o certificado. O `.exe` segue sem assinatura, e o README e a página explicam como abrir apesar do aviso (Mais informações, Executar assim mesmo) e como conferir o arquivo pelo `.sha256.txt`. A assinatura gratuita da SignPath Foundation para código aberto também foi descartada, porque o certificado sairia em nome dela |
