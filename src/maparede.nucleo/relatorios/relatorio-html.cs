...
public static class RelatorioHtml
{
    ...
    /// <summary>Nome sugerido, por exemplo maparede-20260925-1430-192-168-0-0-24.html.</summary>
    public static string NomeArquivo(ResultadoVarredura resultado)
    {
        var rede = resultado.SubRedeVarrida.Rede.ToString().Replace('.', '-');
        return $"maparede-{resultado.Inicio:yyyyMMdd-HHmm}-{rede}-{resultado.SubRedeVarrida.Prefixo}.html";
    }
    ...
    public static string Gerar(ResultadoVarredura r)
    {
        ...
        html.Append($$"""
            ...
            <meta name="generator" content="MapaRede - MT {{C(ResultadoVarredura.VersaoPrograma)}}">
            <title>Mapa de rede {{C(r.SubRedeVarrida.ToString())}} - {{C(Data(r.Inicio))}}</title>
            <style>{{Estilo}}</style>
            </head>
            <body>
            <header>
              <div class="marca">MapaRede - MT</div>
        ...
        html.Append($$"""
            ...
            <footer>
              Relatório gerado pelo <a href="https://maparede.manfred.com.br">MapaRede - MT</a> {{C(ResultadoVarredura.VersaoPrograma)}}, da MT - Manfred Tecnologia.
              Levantamento de inventário: o programa só registra o que a rede mostra a qualquer computador ligado nela.
            </footer>
    ...
    {
        var marcas = h.Marcas;
        var busca = string.Join(' ', h.Ip, h.Nome, h.NomeDns, h.NomeNetBios, h.NomeMdns, h.GrupoNetBios, h.MacTexto, h.Fabricante, string.Join(' ', marcas))
            .ToLowerInvariant();
        ...
        html.Append("<tr class=\"detalhe\" hidden><td colspan=\"6\"><dl>");
