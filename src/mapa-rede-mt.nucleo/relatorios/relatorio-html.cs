...
public static class RelatorioHtml
{
    ...
    /// <summary>Nome sugerido, por exemplo mapa-rede-20260925-1430-192-168-0-0-24.html.</summary>
    public static string NomeArquivo(ResultadoVarredura resultado)
    {
        var rede = resultado.SubRedeVarrida.Rede.ToString().Replace('.', '-');
        return $"mapa-rede-{resultado.Inicio:yyyyMMdd-HHmm}-{rede}-{resultado.SubRedeVarrida.Prefixo}.html";
    }
    ...
    public static string Gerar(ResultadoVarredura r)
    {
        ...
        html.Append($$"""
            ...
            <meta name="generator" content="MT Mapa de Rede {{C(ResultadoVarredura.VersaoPrograma)}}">
            <title>Mapa de rede {{C(r.SubRedeVarrida.ToString())}} - {{C(Data(r.Inicio))}}</title>
            <style>{{Estilo}}</style>
            </head>
            <body>
            <header>
              <div class="marca">MT Mapa de Rede</div>
        ...
        html.Append($$"""
            ...
            <footer>
              Relatório gerado pelo MT Mapa de Rede {{C(ResultadoVarredura.VersaoPrograma)}} - MT - Manfred Tecnologia.
